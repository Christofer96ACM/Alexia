...
function Get_DocumentCorrelative() {
    ...
    var _nls = ((_tdc === "01") ? "FTRSRV/" : (_tdc === "03") ? "BVRSRV/" : "TKRSRV/") + _ser + "-" + txtU_BPP_MDCD.GetText();
...
function OnGetRowValuesProduct(values) {
    ...
    //    txtIssueMthd.SetText(values[7]);
        ...
        var pricelist = cbbListNum.GetValue();
        gdvinv1.PerformCallback("LM:" + values + ":" + pricelist);
...
function OkOdln() {
    ...
    if (rowc === 0) {
        ...
        gdvOdlnSearch.GetRowValues(gdvOdlnSearch.GetFocusedRowIndex(), "DocEntry;GroupNum;SlpCode;Comments;DocCur;OwnerCode;DiscSum;DiscPrcntTotal;ShipToCode;PayToCode;BillStreet;ShipStreet", OnGetRowValuesOdln);
    }
}
...
function OnGetRowValuesOinv(values) {
    ...
    cbbTipoOperacion.SetValue(values[27]);
    ...
    dteDocumentoDate.SetDate(new Date(values[29]));
    ...
    if (hdfTemp.Get("Currency") == "SOL") {
        ...
        bteAnticipo.SetText(new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(values[28]));
    }
    else if (hdfTemp.Get("Currency") == "EUR") {
        txtDiscPrcntTotal.SetText(values[17]);
        ...
        bteAnticipo.SetText(new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format(values[28]));
    }
