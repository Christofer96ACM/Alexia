...
function Get_DocumentType() {
    ...
    $.ajax(
           {
               type: "POST",
               url: "../Sales/ARInvoice.aspx/Get_DocumentType",
...
function DXP_CUENTAS_PAGO() {
    ...
    {
        type: "POST",
        url: "../Sales/ARInvoice.aspx/DXP_CUENTAS_PAGO",
        data: '{"project":"' + project + '", "socied":"' + socied + '"}',
        contentType: "application/json; charset=uft-8",
        dataType: "json",
        success: function (result) {
...
function Set_InitialData(result) {
    ...
    cbbTipoOperacion.SetSelectedIndex(0);
...
function ShowOdlnSearch() {
    if (isNullOrWhiteSpace(txtCardCode.GetText())) {
        ...
        gdvOdlnSearch.PerformCallback("INIT");
        ppcOdlnSearch.Show();
    }
}
function Get_Odln() {
    gdvOdlnSearch.SetFocusedRowIndex(-1);
    gdvOdlnSearch.PerformCallback("GET");
}
function OkOdln() {
    ...
    if (rowc === 0) {
        ...
        gdvOdlnSearch.GetRowValues(gdvOdlnSearch.GetFocusedRowIndex(), "DocEntry;GroupNum;SlpCode;Comments;DocCur;OwnerCode;DiscSum;DiscPrcntTotal;ShipToCode;PayToCode;BillStreet;ShipStreet;SerieGuia", OnGetRowValuesOdln);
    }
}
function OnGetRowValuesOdln(values) {
    hdfTemp.Set("Currency", values[4].toString());
    gdvinv1.PerformCallback("ODLN;" + values);
    gdvAnticipos.PerformCallback("GET;" + values[0] + ";" + 15);
...
function Com_Operation(s, e) {
    ...
    if (e.result.indexOf("SIU") !== -1) {
        ...
        if (cbbDocCur.GetValue().toString() == "USD") {
            txtPendingBal.SetText(new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(_spd[2]));
            txtPaidToDate.SetText(new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(_spd[1]));
        }
        else if (cbbDocCur.GetValue().toString() == "EUR") {
            txtPendingBal.SetText(new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format(_spd[2]));
            txtPaidToDate.SetText(new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format(_spd[1]));
        }
        else {
...
function Get_Totals() {
    ...
    var _ant = parseFloat(alphanumeric(bteAnticipo.GetValue()));
    var _sid = (_sbt - _dst - _ant);
    ...
    if (cbbDocCur.GetValue().toString() == "USD") {
        ...
        bteAnticipo.SetText(new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(_ant.toFixed(2)));
    }
    else if (cbbDocCur.GetValue().toString() == "EUR") {
        ...
        bteAnticipo.SetText(new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format(_ant.toFixed(2)));
    }
    else {
...
function OkOinv() {
    ...
    if (rowc === 0) {
        ...
        gdvOinvSearch.GetRowValues(gdvOinvSearch.GetFocusedRowIndex(), "NumAtCard;DocDate;DocStatus;CardCode;CardName;DocTotal;DocEntry;DocNum;LicTradNum;U_BPP_MDTD;U_BPP_MDSD;U_BPP_MDCD;SlpCode;OwnerCode;GroupNum;Comments;U_BF_PED_ORIG;DiscPrcntTotal;DiscSum;TotalExpns;PaidToDate;DocCur;ListNumSrch;ShipToCode;PayToCode;BillStreet;ShipStreet;SerieGuia;U_DXP_FE_TO;DpmAmnt;TaxDate", OnGetRowValuesOinv);
    }
}
...
function OnGetRowValuesOinv(values) {
    ...
    if (hdfTemp.Get("Currency") == "SOL") {
        ...
        bteAnticipo.SetText(new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(values[29]));
    }
    else if (hdfTemp.Get("Currency") == "EUR") {
        txtDiscPrcntTotal.SetText(values[17]);
        ...
        bteAnticipo.SetText(new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format(values[29]));
    }
...
function ShowOrdrSearch() {
    if (isNullOrWhiteSpace(txtCardCode.GetText())) {
        ...
        gdvOrdrSearch.PerformCallback("INIT");
        ppcOrdrSearch.Show();
    }
}
function Get_Ordr() {
    gdvOrdrSearch.SetFocusedRowIndex(-1);
    gdvOrdrSearch.PerformCallback("GET");
}
function OkOrdr() {
    ...
    if (rowc === 0) {
        ...
        gdvOrdrSearch.GetRowValues(gdvOrdrSearch.GetFocusedRowIndex(), "DocEntry;GroupNum;SlpCode;Comments;DocCur;OwnerCode;DiscSum;DiscPrcntTotal;ShipToCode;PayToCode;BillStreet;ShipStreet", OnGetRowValuesOrdr);
    }
}
function OnGetRowValuesOrdr(values) {
    hdfTemp.Set("Currency", values[4].toString());
    gdvinv1.PerformCallback("ORDR;" + values[0]);
    gdvAnticipos.PerformCallback("GET;" + values[0] + ";" + 17);
    txtShipToCode.SetText(values[8]);//ENTREGA
    bteShipToCode.SetText(values[11]);//ENTREGA
    txtBillToCode.SetText(values[9]);//FACTURA
    bteBillToCode.SetText(values[10]);//FACTURA
    cbbGroupNum.SetValue(values[1]);
    //alert("" + hdfTemp.Get("totalAnt"));
    if (values[5] != "0") cbeOwnerCode.SetValue(values[5]);
    else cbeOwnerCode.SetText("");
    ...
    if (hdfTemp.Get("Currency") == "SOL") {
        ...
        txtTotalExpns.SetText(new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format("0"));
        txtPaidToDate.SetText(new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format("0"));
        bteAnticipo.SetText(new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format("0"));
    }
    else if (hdfTemp.Get("Currency") == "EUR") {
        txtDiscPrcntTotal.SetText(values[7]);
        txtDiscSum.SetText(new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format(values[6]));
        txtTotalExpns.SetText(new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format("0"));
        txtPaidToDate.SetText(new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format("0"));
        bteAnticipo.SetText(new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format("0"));
    }
    cbbDocCur.SetEnabled(false);
...
function ShowOqutSearch() {
    if (isNullOrWhiteSpace(txtCardCode.GetText())) {
        ...
        gdvOqutSearch.PerformCallback("INIT");
        ppcOqutSearch.Show();
    }
}
function Get_Oqut() {
    gdvOqutSearch.SetFocusedRowIndex(-1);
    gdvOqutSearch.PerformCallback("GET");
}
function OkOqut() {
    ...
    if (rowc === 0) {
        ...
        gdvOqutSearch.GetRowValues(gdvOqutSearch.GetFocusedRowIndex(), "DocEntry;GroupNum;SlpCode;Comments;DocCur;OwnerCode;DiscSum;DiscPrcntTotal;ShipToCode;PayToCode;BillStreet;ShipStreet", OnGetRowValuesOqut);
    }
}
function OnGetRowValuesOqut(values) {
    hdfTemp.Set("Currency", values[4].toString());
    gdvinv1.PerformCallback("OQUT;" + values[0]);
    gdvAnticipos.PerformCallback("GET;" + values[0] + ";" + 23);
    txtShipToCode.SetText(values[8]);//ENTREGA
    bteShipToCode.SetText(values[11]);//ENTREGA
    txtBillToCode.SetText(values[9]);//FACTURA
    bteBillToCode.SetText(values[10]);//FACTURA
...
var sum_anticipos = 0;
var index = 0;
function EndgdvAnticipos() {
    var a = gdvAnticipos.GetVisibleRowsOnPage();
    if (a > 0) {
        for (index = 0; index < a ; index += 1) {
            gdvAnticipos.GetRowValues(index, "DocEntry;Monto;Active", SumaAnticipos);
        }
    }
    else {
        if (hdfTemp.Get("Currency") == "SOL") {
...
function SumaAnticipos(values) {
    if (values[2] === "Y") {
        if (hdfTemp.Get("Currency") == "SOL") {
            bteAnticipo.SetText(parseFloat(bteAnticipo.GetText()) + parseFloat(values[1]));
        }
        else if (hdfTemp.Get("Currency") == "USD") {
            bteAnticipo.SetText(new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(parseFloat(bteAnticipo.GetText()) + parseFloat(values[1])));
        }
        else if (hdfTemp.Get("Currency") == "EUR") {
            bteAnticipo.SetText(new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format(parseFloat(bteAnticipo.GetText()) + parseFloat(values[1])));
        }
        
    }
    //alert("" + values[0] + "-" + values[1] + "-" + values[2] + "-" + (sum_anticipos + parseFloat(values[1])) + "--" + sum_anticipos);
    ///bteAnticipo.SetText(parseFloat(values[1]));
}
function OK_Anticipos() {
    bteAnticipo.SetText("0");
    EndgdvAnticipos();
    ppcAnticipos.Hide();
}
function CancelAnticipos() {
    ppcAnticipos.Hide();
}
