...
function Get_InitialData() {
    ...
    {
        type: "POST",
        url: "../Purchase/CompraFactura.aspx/Get_InitialData",
        data: '{"socied":"' + socied + '"}',
        contentType: "application/json; charset=uft-8",
        dataType: "json",
        success: function (result) {
...
function OkOdln() {
    ...
    if (rowc === 0) {
        ...
        gdvOdlnSearch.GetRowValues(gdvOdlnSearch.GetFocusedRowIndex(), "DocEntry;GroupNum;SlpCode;Comments;DocCur;OwnerCode;DiscSum;DiscPrcntTotal", OnGetRowValuesOdln);
    }
}
...
function OnGetRowValuesOinv(values) {
    ...
    txtShipToCode.SetText(values[23]);
    bteShipToCode.SetText(values[26]);
    txtBillToCode.SetText(values[24]);
    bteBillToCode.SetText(values[25]);
...
function Med_Payment2() {
    if (Get_OutBalance() !== 0) {
        ...
        txtSalEfe3.SetText("0");
        txtSalTar3.SetText("0");
        txtSalTrans3.SetText("0");
        dteDocDateTrans.SetDate(new Date());
        //}
        cbbCreditCard.SetValue(_vld[5]);
        txtCardNum.SetText("");
