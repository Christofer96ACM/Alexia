...
function Get_InitialData() {
    ...
    {
        type: "POST",
        url: "../Sales/OrdenVenta.aspx/Get_InitialData",
        data: '{"socied":"' + socied + '"}',
        contentType: "application/json; charset=uft-8",
        dataType: "json",
        success: function (result) {
...
function OkProduct() {
    ...
    if (rowc === 0) {
        ...
        gdvProductSearch.GetRowValues(gdvProductSearch.GetFocusedRowIndex(), "ItemCode;ItemName;OnHand;PriceBefDi;DiscPrcnt;Price;PriceVat;NumInSale;IssueMthd;ActivoC;ActivoS", OnGetRowValuesProduct);
    }
}
...
// CANCELAR
function Cancel() {
    var result = confirm("¿Desea cancelar la Orden de venta?");
    if (result) {
...
function OkOqut() {
    ...
    if (rowc === 0) {
        ...
        gdvOqutSearch.GetRowValues(gdvOqutSearch.GetFocusedRowIndex(), "DocEntry;DocNum;DocTotalSrch;ListNumSrch;DocCur;SlpCode;OwnerCode;ShipToCode;PayToCode;Address;Address2;GroupNum;DiscPrcntTotal;DiscSum;Comments", OnGetRowValuesOqut);
    }
}
