...
function OnGetSelectedFieldValues(selectedValues) {
    if (selectedValues.length == 0) {
        ...
        if (csc === 1/* && cbc === 1*/) {
            ...
            alert("No se permite seleccionar mas de un tipo de dirección.");
        }
    }
}
...
function Set_InitialData(result) {
    ...
    $.each(_rs3, function (index, item) {
        cbbMotTras.InsertItem(index, item.Name, item.Code);
    });
    cbbMotTras.EndUpdate();
    cbbMotTras.SetSelectedIndex(0);

    var _rs4 = result.d.where(function (item) { return (item.Ident === "C20"); });
    cbbModTras.ClearItems();
    cbbModTras.BeginUpdate();
    $.each(_rs4, function (index, item) {
        cbbModTras.InsertItem(index, item.Name, item.Code);
    });
...
function Set_DeliveryNote(result) {
    if (result.d === null) {
        ...
        if (result.d.SlpCode != "-1" || result.d.SlpCode != 0) cbeSalesPerson.SetValue(result.d.SlpCode);
...
function Get_DocumentCorrelative() {
    ...
    var _nls = "GR/" + _ser + "-" + txtU_BPP_MDCD.GetText();
...
function OkProduct() {
    ...
    if (rowc === 0) {
        ...
        gdvProductSearch.GetRowValues(gdvProductSearch.GetFocusedRowIndex(), "ItemCode;ItemName;OnHand;PriceBefDi;DiscPrcnt;Price;PriceVat;IssueMthd;NumInSale;ActivoC;ActivoS;InvntItem;DetalleServicio", OnGetRowValuesProduct);
    }
}
...
function OnGetRowValuesProduct(values) {
    ...
    //    txtIssueMthd.SetText(values[7]);
        ...
        var pricelist = cbbListNum.GetValue();
        gdvdln1.PerformCallback("LMPRODUCT:" + values + ":" + pricelist);
...
function OkOdln() {
    ...
    if (rowc === 0) {
        ...
        gdvodlnSearch.GetRowValues(gdvodlnSearch.GetFocusedRowIndex(), "NumAtCard;DocDate;DocStatus;CardCode;CardName;DocTotalSrch;DocEntry;DocNum;LicTradNum;U_BPP_MDTD;U_BPP_MDSD;U_BPP_MDCD;SlpCode;OwnerCode;GroupNum;Comments;U_BF_PED_ORIG;DiscPrcntTotal;DiscSum;TotalExpns;PaidToDate;ShipToCode;PayToCode;BillStreet;DocCur;ListNumSrch;ShipStreet;TaxDate;U_DXP_FE_MOTIVO_TRAS;U_DXP_FE_MODALIDTRAS", OnGetRowValuesOdln);
    }
}

function OnGetRowValuesOdln(values) {

    cbbMotTras.SetValue(values[28]);
    cbbModTras.SetValue(values[29]);
    txtShipToCode.SetText(values[21]);//ENTREGA
    bteShipToCode.SetText(values[26]);//ENTREGA
    txtBillToCode.SetText(values[22]);//FACTURA
    bteBillToCode.SetText(values[23]);//FACTURA
