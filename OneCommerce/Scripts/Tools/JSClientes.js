...
function Show_ProvinciaSearch() {
    if (!isNullOrWhiteSpace(State.GetText())) {
        var prov = txtProvincia.GetText();
        gdvProvincia.PerformCallback("SRCH:" + State.GetText() + ":" + prov);
...
function Show_DistritoSearch() {
    if (!isNullOrWhiteSpace(U_DXP_BIZ_COPR.GetText())) {
        ...
        gdvDistrito.PerformCallback("SRCH:" + U_DXP_BIZ_COPR.GetText() + ":" + dist);
...
function Show_Gabinete() {
    gdvGabinete.PerformCallback("SRCH");
    ppcGabineteSearch.Show();
}
function OkGabinete() {
    var rowc = gdvGabinete.GetVisibleRowsOnPage();
    if (rowc === 0) {
        alert("No existen registros para realizar esta operación.")
    }
    else if (gdvGabinete.GetFocusedRowIndex() === -1) {
        alert("Seleccione un registro para realizar esta operación.")
    }
    else {
        gdvGabinete.GetRowValues(gdvGabinete.GetFocusedRowIndex(), "Code;Name", OnGetRowValuesgabi);
    }
}

function OnGetRowValuesgabi(values) {
    U_DXP_BIZ_ZONA.SetText(values[0]);
    gdvGabinete.SetFocusedRowIndex(-1);
    ppcGabineteSearch.Hide();
}
function EndGabineteSearch() {
    gdvGabinete.SetFocusedRowIndex(0);
}
...
function OnGetRowValuesProvincia(values) {
    U_DXP_BIZ_COPR.SetText(values[0]);
    U_DXP_BIZ_PROV.SetText(values[1]);
...
function OnGetRowValuesDistrito(values) {
    U_DXP_BIZ_CODI.SetText(values[0]);
    U_DXP_BIZ_DIST.SetText(values[1]);
