...
{
    public partial class ARInvoice : Page
    {
        ...
        public static List<BEDocumentType> Get_DocumentType(string project, string socied)
        {
            ...
            var msbl = new string[] { "07", "09", "99" , "TN"};
        ...
        protected void gdvOdlnSearch_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (e.Parameters.Contains("GET"))
            {
                ...
                var obrd = new BRDocument();
                var olst = obrd.Get_OSCSP_SRCH_ODLN2(obj);
                Session["odln"] = olst;
        ...
        protected void gdvinv1_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (e.Parameters.Contains("QK"))
            {
                ...
                var _qtk = (true) ? Convert.ToInt32(txtQuantityKit.Text) : Convert.ToInt32(txtQKit.Text);
                olst.ForEach(item =>
                {
                    var line = new BEDocumentLine()
            ...
            else if (e.Parameters.Contains("LMPRODUCT"))
            {
                var band = true;
                var suprimir = "";
                var _spl = e.Parameters.Split(':');
            ...
            else if (e.Parameters.Contains("ORDR"))
            {
                ...
                var obrd = new BRDocument();
                //var olst = obrd.GET_ANTICIPOS(obep);
                //olst.ForEach(i =>
                //{
                //    hdfTemp.Set("totalAnt", Convert.ToDouble(hdfTemp.Get("totalAnt"))+ Convert.ToDouble(i.Monto));
                //});
            ...
            else if (e.Parameters.Contains("ODLN"))
            {
                ...
                var obrd = new BRDocument();
                //var olst = 
                //Session["inv1"] = obrd.Get_OSCSP_SRCH_DLN12(obep);
                var list = obrd.Get_OSCSP_SRCH_DLN12(obep);
                list.ForEach(item => {
                    if (item.TreeType == "S")
                    {
                        item.Father = item.ItemCode;
            ...
            if (string.IsNullOrWhiteSpace(_err))
            {
                ...
                //{
                    ...
                    using (var obrd = new BRDocument())
                    {
                            if (band)
                            {
                                obrd.SaveDraft(obj, ((BEParameters)Session["InitPar"]).objSapSbo, 13);
                            }
                            else
        ...
        private string CreditNote()
        {
            ...
            obj.Comments = mmoComments.Text;
            obj.Reserve = "N";
            obj.U_BPP_MDTO = cbbU_BPP_MDTD.Value.ToString();
            obj.U_BPP_MDSO = cbbU_BPP_MDSD.Text;
            obj.U_BPP_MDCO = txtU_BPP_MDCD.Text;
            obj.ShipToCode = txtShipToCode.Text;
            obj.PayToCode = txtBillToCode.Text;
                ...
                if (item.IssueMthd.Substring(0,1) != "B" && item.TreeType != "S")
                {
                    ...
                    using (var obrd = new BRDocument())
                    {
                        ...
                        if (item.IssueMthd.Substring(0, 1) == "M")
                            ...
                            seriesUsadas = obrd.GET_LOTES_USADAS(obep);
                    }
        ...
        private string Imprimir()
        {
            ...
            obj.NumAtCard = txtNumAtCard.Text;
            obj.U_BPP_MDTD = cbbU_BPP_MDTD.Value.ToString();
            if(obj.U_BPP_MDTD == "01" || obj.U_BPP_MDTD == "TK")
                obj.formatoImpresion = "FORMATOFACTURA";
            else if(obj.U_BPP_MDTD == "08")
                obj.formatoImpresion = "FORMATONOTADEBITO";
        ...
        protected void gdvOqutSearch_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (e.Parameters.Contains("GET"))
            {
                var obj = new BEParameters()
                {
                    ...
                    Project = obec.Project,
                    CardCode = txtCardCode.Text,
                    NumAtCard = txtoqDocNum.Text.Trim(),
                    DateIn = (DateTime?)dteoqDateIn.Value,
                    DateFi = (DateTime?)dteoqDateFi.Value
                };
                var obrd = new BRDocument();
